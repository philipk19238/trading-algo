...
class LabelService:
    ...
    def get_starting():
        ...
        if not start_idx: 
            return len(pd.read_csv('labeled_data.csv'))
...
if __name__ == '__main__':
    ...
    with open('wsbData.json', 'r') as f: 
        ...
        while curr and cont: 
            body = curr.get('body')
            label = input(f"Enter 'y' for Bull, 'n' for Bear, and blank for Neutral\n\n{body}\n\n")
    ...
    if 'labeled_data.csv' in os.listdir(os.getcwd()):
        prev = pd.read_csv('labeled_data.csv')
